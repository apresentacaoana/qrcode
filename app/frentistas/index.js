...
const FrentistasPage = () => {
    ...
    useEffect(() => {
        async function getData() {
            ...
            setData(response.filter((user) => user.role === 'frentista'))
            setBackup(response.filter((user) => user.role === 'frentista'))
        }
        getData()
    }, [])
        ...
        <View className="mt-8" />
            ...
            <View className="flex-1 m-7">
                <View className="flex flex-row mb-10">
                    ...
                    <Text className="text-[#0f0d3c] ml-3 text-[28px]">Frentistas</Text>
                </View>
                ...
                <ScrollView className="flex-1">
                    <View className="mt-3">
                            {data.map((object) => (
                                <TouchableOpacity key={object.docId} className={"mb-2"} onPress={() => router.push(`/frentista/${object.id}`)}>
                                   <Widget className={"flex flex-row mb-1 items-center justify-between"} variant={"filled"}>
                                        ...
                                        <View className="flex ml-5 flex-row justify-between items-center grow">
                                            <View>
                                                ...
                                                <Text className=" text-white">{object.posto.nome}</Text>
                                            </View>
                                            <View>
