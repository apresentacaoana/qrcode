...
import { getUserById, newMessage } from '../db/service'
...
const SuporteChat = () => {
    ...
    async function send() {
      await newMessage({message: content, from: user.docId, to: userSupport.docId})
      setContent('')
    }
    ...
    const admin = (message) => (
      <View className='self-start mt-2'>
        <View key={message.id} className=' flex flex-col w-fit self-start p-3 rounded-[20px] rounded-bl-none px-5 border border-gray-300 bg-gray-200 '>
          <Text className=' uppercase self-start text-black font-semibold text-[13px]'>{userSupport.nome}</Text>
    ...
    const eu = (message) => (
      ...
      <View className='self-end mt-2'>
        ...
        <View key={message.id} className=' flex flex-col w-fit self-end p-3 rounded-[20px] rounded-br-none px-5 border border-gray-300 bg-yellow-100 '>
          <Text className=' uppercase self-start text-black font-semibold text-[13px]'>ADMIN</Text>
          <Text className=' text-[19px] text-gray-500'>{message.message}</Text>
        </View>
          ...
          <View className='flex-1 m-7'>
            ...
            <ScrollView ref={scrollViewRef}>
              {messages.length > 0 && messages.map((message) => {
                console.log(message)
                console.log(`Essa mensagem foi enviada por mim? ${message.from == user.docId}`)
                console.log(`Essa mensagem foi enviada para esse cliente? ${message.to == userSupport.docId}`)
                console.log(`ID do Cliente: ${userSupport.docId}`)
                console.log(`ID do Receptor da mensagem: ${message.to}`)
                if((message.from == user.docId && message.to == userSupport.docId)) {
                  return eu(message)
                } else if (message.to == 'admin' && message.from == userSupport.docId) {
