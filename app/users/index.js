...
const UsersPage = () => {
    ...
    useEffect(() => {
        async function getData() {
            ...
            setData(response.filter((user) => user.role === 'normal'))
            setBackup(response.filter((user) => user.role === 'normal'))
        }
        getData()
    }, [])
        ...
        <View className="mt-8" />
            ...
            <View className="flex-1 m-7">
                <View className="flex flex-row mb-10">
                    ...
                    <Text className="text-[#0f0d3c] ml-3 text-[28px]">Usuários</Text>
                </View>
                ...
                <ScrollView className="flex-1">
                    <View className="mt-3">
                            {data.map((object) => (
                                <TouchableOpacity key={object.docId} className={"mb-2"} onPress={() => router.push(`/edit/user/${object.id}`)}>
